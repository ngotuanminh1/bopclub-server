	...
	if (dice) {
		...
		TXCuoc.find({phien:game_id}, null, {sort:{'_id':-1}}, function(err, list) {
			if(list.length){
				...
				let TaiXiu_tong_red_lech = Math.abs(TaiXiu_red_tong_tai - TaiXiu_red_tong_xiu); //Sửa 11/5
				let TaiXiu_red_lech_tai  = TaiXiu_red_tong_tai > TaiXiu_red_tong_xiu;
	...
	gameLoop = setInterval(function(){
		...
		if (io.TaiXiu_time <= 60) {
			if (io.TaiXiu_time < 0) {
				...
				if (!!botList.length && io.TaiXiu_time === 59) {
					bot.tx(botList, io, 49000); // Rải đều cược trong 59 giây
					botList = []; // Tránh cược lại
				}
			}
		}
		botHu(io, botTemp);
